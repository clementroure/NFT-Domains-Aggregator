...
const typeOfProperty = [
  {
    name: "ENS",
    description: "Most widely integrated blockchain naming standard.",
    checked: true,
  },
  {
    name: "Unstoppable Domains",
    description: "Lifetime blockchain domains.",
    checked: true,
  },
];
...
const PropertyTypeSelect: FC<PropertyTypeSelectProps> = ({
  ...
  const [ENS, setENS] = useState(true);
  const [UD, setUD] = useState(true);
        ...
        <>
          ...
          >
            ...
            <div className="flex-grow">
              <span className="block xl:text-lg font-semibold">{(!ENS && !UD) ? "No Provider" : ((ENS && UD) ? "ENS & UD" : (ENS && !UD) ? "ENS" : "UD")}</span>
                  ...
                  {typeOfProperty.map((item, index) => (
                    <div key={item.name} className="">
                      <Checkbox
                        ...
                        defaultChecked={ index == 0 ? ENS : UD}
                        onChange={(checked) =>
                          index == 0 ? setENS(checked) : setUD(checked)
                        }
                      />
                    </div>
                  ))}
